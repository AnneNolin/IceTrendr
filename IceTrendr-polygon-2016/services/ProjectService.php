...
DOQ;
	...
	public function addProjectTsa($project_id, $tsa) {
		...
		mysqli_stmt_bind_param($stmt, 'ii', $project_id, $tsa);
		$this->throwExceptionOnError();
...
DOQ;
		...
		mysqli_stmt_bind_param($stmt, 'iii', $project_id, $tsa, $interpreter);
		$this->throwExceptionOnError();
	 ...
	 * @return array
	 */
	public function getUserTasks($interpreter) {
		$sql = <<<DOQ
			SELECT projects.project_id, project_code, project_name, 
						 description, contact, tsa, user_id
			...
			AND project_interpreter.status = 1
