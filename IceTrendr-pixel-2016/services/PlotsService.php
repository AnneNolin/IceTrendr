	...
	public function getAllPlots($project_id, $tsa, $interpreter) {
		...
		$sql =<<<DOQ
			SELECT plots.project_id, plots.tsa, plots.plotid, 
				 x, y, lat, lng, 
				 is_forest, is_tiger_urban, is_complete, is_example
...
DOQ;
	...
	private function getPlotsByID($itemID) {
		...
		mysqli_stmt_bind_result($stmt, $row->plotid, $row->x, $row->y, $row->is_complete, $row->is_example, $row->comment);
	...
	private function createPlots($item) {
		$this->connect();
		$stmt = mysqli_prepare($this->connection, "INSERT INTO $this->tablename (plotid, x, y, is_complete, is_example, comment) VALUES (?, ?, ?, ?, ?, ?)");
		$this->throwExceptionOnError();

		mysqli_stmt_bind_param($stmt, 'iddiis', $item->plotid, $item->x, $item->y, $item->is_complete, $item->is_example, $item->comment);
		$this->throwExceptionOnError();
	...
	private function updatePlots($item) {
		$this->connect();
		$stmt = mysqli_prepare($this->connection, "UPDATE $this->tablename SET plotid=?, x=?, y=?, is_complete=?, is_example=?, comment=? WHERE plotid=?");		
		$this->throwExceptionOnError();
		
		mysqli_stmt_bind_param($stmt, 'iddiisi', $item->plotid, $item->x, $item->y, $item->is_complete, $item->is_example, $item->comment, $item->plotid);		
		$this->throwExceptionOnError();
