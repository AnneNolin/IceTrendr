...
; PURPOSE:
;
;  This function create a standard set of historical variables from Icetrendr output.
...
;******************************************************************************************;
FUNCTION it_history_metrics, diagfile, end_year=end_year, start_year=start_year, run=run, $
  ...
  IF n_elements(suffix) EQ 0 THEN suffix = 'itmetrix'
  ...
  ;diag_file = file_search(it_outputs_path+path_sep()+'*_'+run+'_diag.sav', count=n)
  ;IF n NE 1 THEN message, 'Diag file not found.'
  
  ;sv_file = file_search(it_outputs_path+path_sep()+'*'+run+'_fitted.bsq', count=n)
  ;IF n NE 1 THEN message, 'Fitted file not found.'
  
  ;se_file = file_search(it_outputs_path+path_sep()+'*'+run+'_segmse.bsq', count=n)
  ;IF n NE 1 THEN message, 'Segmse file not found.'
  
;  vv_file = file_search(it_outputs_path+path_sep()+'*_'+run+'_vertvals.bsq', count=n)
;  IF n NE 1 THEN message, 'Vertvals file not found.'
;  
;  vy_file = file_search(it_outputs_path+path_sep()+'*_'+run+'_vertyrs.bsq', count=n)
;  IF n NE 1 THEN message, 'Vertyrs file not found.'
;  
;  ftv_tcb_file = file_search(it_outputs_path+path_sep()+'*'+run+'brightness_ftv_fitted.bsq', count=n)
;  IF n NE 1 THEN message, 'FTV brightness file not found.'
;  
;  ftv_tcg_file = file_search(it_outputs_path+path_sep()+'*'+run+'greenness_ftv_fitted.bsq', count=n)
;  IF n NE 1 THEN message, 'FTV greenness file not found.'
;  
;  ftv_tcw_file = file_search(it_outputs_path+path_sep()+'*'+run+'wetness_ftv_fitted.bsq', count=n)
...
if current_chunk eq n_chunks then return, {ok:1, hist_info:hist_info, msg:'Already completely processed'}	;set here so we don't reprocess everything. 
  ...
  ;    src_tcb_file = file_search(it_outputs_path+path_sep()+'*'+run+'brightness_ftv_source.bsq', count=n)
  ;    IF n NE 1 THEN message, 'FTV brightness file not found.'
  ;
  ;    src_tcg_file = file_search(it_outputs_path+path_sep()+'*'+run+'greenness_ftv_source.bsq', count=n)
  ;    IF n NE 1 THEN message, 'FTV greenness file not found.'
  ;
  ;    src_tcw_file = file_search(it_outputs_path+path_sep()+'*'+run+'wetness_ftv_source.bsq', count=n)
  ;    IF n NE 1 THEN message, 'FTV wetness file not found.'
  ;
