retall
;this batch file controls all of the ledaps icetrendr preprocessing steps

;---please state the landsat path/row ID and path to the data as described following each variable---
ppprrr = '046026' ; ex. '046026' -MUST BE PPP (path) and RRR (row), three digits for both path and row, use zero if needed ex: '0PP0RR'
path = '/mnt/IceTrendr/scenes/046026/' ; ex. 'F:\046026\' -MAKE SURE THERE IS A "\" AT THE END, must point to a drive path, even if on a server  
useareafile = "/mnt/IceTrendr/scenes/046026/images/_usearea.bsq" ; full path to a 0-1 raster that defines where Icetrendr will be run :: zeros = don't run, ones = run 
segparamstxt = "/mnt/IceTrendr/code/helperfiles/nbr_segmentation_parameters.txt" ;give full path to the segmentation parameter .txt file
template_hdr = "/mnt/IceTrendr/code/helperfiles/mrlc_template_headerfile.hdr"  ;give the full path to a template projection header (.hdr) file
label_parameters_txt = "/mnt/IceTrendr/code/helperfiles/eval_label_params.txt"
class_code_txt = "/mnt/IceTrendr/code/helperfiles/eval_class_codes.txt"
...
segmentation_eval    = 1  ;runs icetrendr in evaluation mode
...
.run tbcd_v2
ledaps_icetrendr_processor, run_params
