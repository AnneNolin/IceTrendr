...
;the value required for IT masks and renames and moves
...
pro create_it_masks_from_vct_masks, path, ppprrr, proj, templatehdr, outpath=outpath, water_off=water_off,$
  ...
  vct_output_path = strcompress(path+"VCT\outputs\", /rem)
  it_img_path = strcompress(path+'images\', /rem)
  ...
  for i=0, n_elements(m_files)-1 do begin
    ...
    ;---convert month-day in the file names to julian day so we can get the names into IT convention---
    ...
    if vct_substr.sensor_nbr eq 7 then sensor_name = 'LE' else sensor_name = 'IT'
    
    ;---create it mask outputs---
    ...
    outfile = strcompress(it_img_path+vct_substr.year+'\'+outfilebase, /rem)
    exists = file_search(it_img_path+vct_substr.year+'\', strcompress("*"+part1+"*"+part3+"*", /rem), count=n_exists)
    ...
    ;delete old ones
    it_delete_duplicate, outfile, /cloudmask
    
    dir = strcompress(it_img_path+vct_substr.year+'\', /rem)
    tempfiles = file_search(dir, "*temp*", count=count)
    if count ge 1 then file_delete, tempfiles, /quiet
    
    temp_out = strcompress(it_img_path+vct_substr.year+'\temp.bsq', /rem)
    ...
    temp_shdwmsk = strcompress(it_img_path+vct_substr.year+'\temp_shdwmsk.bsq', /rem)
    openw, un, temp_shdwmsk, /get_lun
    writeu, un, shadowedge
    free_lun, un
    write_im_hdr, temp_shdwmsk, hdr
    convert_bsq_headers_to_envi, strcompress(it_img_path+vct_substr.year, /rem), templatehdr
