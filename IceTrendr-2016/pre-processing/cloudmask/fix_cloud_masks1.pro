...
pro fix_cloud_masks1, path, ppprrr, image_info_savefile, fixmask=fixmask
  ...
  for i=0, n_elements(dothese)-1 do begin
    ...
    window, 0, xsize=1000, ysize=1000, title=title
    ...
    if areclouds eq '2' then begin
      ...
      repeat begin
        ...
        for i=0, 5 do begin
          title = file_basename(imgmatch)
          if i eq 0 then window, 0, xsize=1000, ysize=1000, title=title;create a window to hold all of the regression tifs
          if i gt 0 then erase
          tv, img321, 0, true=3
          wait, .8
          ;window, 0, xsize=1000, ysize=1000, title=title;create a window to hold all of the regression tifs
          erase
          tv, img321m, 0, true=3
          wait, .8
        endfor
