...
pro fix_cloud_masks, path, ppprrr, image_info_savefile, fixmask=fixmask, from_madcal=from_madcal
...
;    gapimg = temporary(gapimg) ne 0
    ...
    img321 = reverse(hist_equal(congrid(goods, xsize, ysize, 3),  minv=10, maxv=175),2) ; reverse,2
    
    reload1:
    title = file_basename(imgmatch)
    window, 0, xsize=640, ysize=640, title=title
    tv, img321, 0, true=3
    wait, 1
    repeat begin
      print, ">>>"
    ...
    if areclouds eq '2' then begin
      ...
      repeat begin
        for r=0, 2 do begin
          ...
          if n_thermal eq 1 then goods[*,*,r] = img321m*(maskimg ge adj) else goods[*,*,r] = img321m*(maskimg le adj)
        endfor
        img321m = reverse(hist_equal(congrid(goods, xsize, ysize, 3),  minv=10, maxv=175),2) ;,  minv=10, maxv=175
        
        reload2:
        title = file_basename(imgmatch)
        window, 0, xsize=1280, ysize=640, title=title;create a window to hold all of the regression tifs
        tv, img321, 0, true=3
        tv, img321m, 1, true=3
        wait, 1
        repeat begin
        ...
        if status eq '4' then begin
          if n_thermal eq 1 then begin
            ...
            repeat begin
              for r=0, 2 do begin
                zot_img, imgmatch, hdr, img321m, layers = 3-r, subset=subset
                img321m[where(maskimg le adj)] = 0
                img321m[where(maskimgadded ge adj1)] = 0
                goods[*,*,r] = img321m
              endfor
    ...
    if areshadows eq '2' then begin
      ...
      repeat begin
        for r=0, 2 do begin
          ...
          img543m[where(maskimg le adj)] = 255
          goods[*,*,r] = img543m
        endfor
    ...
    for r=0, 2 do begin
      ...
      if r eq 0 then img543m[where(finalcloudmask eq 0)] = 0
      if r eq 1 then img543m[where(finalcloudmask eq 0)] = 0
      if r eq 2 then img543m[where(finalcloudmask eq 0)] = 0
      goods[*,*,r] = img543m
    endfor
    ;img2 = goods
    img543m = reverse(hist_equal(congrid(goods, xsize, ysize, 3), minv=10, maxv=175),2) ;,
    goods = 0 ;free memory
    ...
    repeat begin
      print, ">>>"
      print, ">>> the black is the final mask"
      print, ">>> is it OK???"
      print, ">>>"
      print, ">>> NO (press 1), YES (press 2)"
      status = ''
    ...
    it_delete_duplicate, imgmatch, /cloudmask
    
    ;changing \ to generic dir on Feb 28, 2013
    
    filedir = file_dirname(imgmatch)+path_sep()
