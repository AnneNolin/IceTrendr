...
def it_basename(basename, appendix='_ledaps.bsq', with_ts=True):
	...
	if 'LE7' in basename:
		...
		return "LE7"+basename[3:9]+'_'+basename[9:13]+'_'+basename[13:17]+'_'+ts+appendix
	if 'LT5' in basename:
...
def stack_bands(gtif_dir, output_path, proj_file):
    #find image year information and make a year output folder
    Band_files = glob.glob(gtif_dir + '/*_band*.tif')
    Band_files = sorted(Band_files)

    basefile = os.path.splitext(os.path.basename(Band_files[0]))[0][0:16]
    print basefile
    if 'L7' in basefile:
	...
	print it_basename(basefile, '', False)
	return
    else:
	print "start processing"

    refl_file = gtif_dir + '/' + basefile + '_stacked_bands.bsq'
    print(refl_file)

    merge_cmd = "gdal_merge.py -o {0} -of envi -separate -n -9999 {1}"

    print("stacking images to create " + refl_file)
    os.system(merge_cmd.format(refl_file, ' '.join(Band_files[0:6])))

    #reproject to albers
    warp_cmd = 'gdalwarp -of ENVI -t_srs '+ proj_file +' -tr 30 30 -srcnodata "-9999 0" -dstnodata "-9999" {0} {1}'
    this_refl = basefile[0:9] + '_' + basefile[9:13] + '_' + basefile[13:16] + '_ledaps.bsq'
    print("reprojecting to " + this_refl)
    os.system(warp_cmd.format(refl_file, os.path.join(this_outputdir, this_refl)))
...
def extract_hdf(hdf_file, output_path, proj_file, temp_path=''):
	...
	#reproject to albers
	warp_cmd = 'gdalwarp -of ENVI -t_srs '+ proj_file +' -tr 30 30 -srcnodata "-9999 0" -dstnodata "-9999" {0} {1}'
	this_refl = it_basename(basefile, '_ledaps.bsq')
	print("reprojecting to " + this_refl)
...
def processLedaps2(gtif_dir, output_path, proj_file, temp_path=''):
	"""process all the ledaps hdf files in the specified directory

		ledapsdir: input directory of separate bands of geotiff files
		output_path: output directory for image stacks
	"""
	failed = []
	stack_bands(gtif_dir, output_path, proj_file)
	"""
	try:
	    print("Stacking bands in " + targz_dir)
	    stack_bands(targz_dir, output_path, proj_file, tmp_path='')
	except:
	    import pdb; pdb.set_trace()
...
def create_ledaps_tc2(refl_file, tc_type):
	...
	if (tc_type == 'sr'):
	    ...
	    brt_coeffs = [ 0.33183,  0.33121,  0.55177,  0.42514,  0.48087,  0.25252]
	    grn_coeffs = [-0.24717, -0.16263, -0.40639,  0.85469,  0.05493, -0.11749]
	    wet_coeffs = [ 0.13929,  0.22490,  0.40359,  0.25178, -0.70133, -0.45732]
	else:
	    print("unrecognized tc_type, Only tc_type 'sr' and 'toa' allowed.") 
	    return 0
	...
	tc_file = refl_file.replace('_ledaps.bsq', '_tc.bsq')
...
def processIcetrendrTC2(img_dir, out_dir, tc_type):
	...
	for directory, dirnames, filenames in os.walk(img_dir):
		for f in filenames:
			if (f.find('_cfmask.tif') < 0): continue
			this_basename = f[0:9]
			this_year = f[9:13]
			this_day = f[13:16]
			this_output = os.path.join(out_dir, this_year)
			if not os.path.exists(this_output):
				print("Error in processIcetrendrTC2: " + f + " does not exist")
				return 0

			this_file = os.path.join(this_output, this_basename + '_' + this_year + '_' + this_day + '_ledaps.bsq')
			try:
				print("TC creation:Processing " + this_file + ' using \'sr\' coefficients')
				create_ledaps_tc2(this_file, tc_type)
			except:
				print(sys.exc_info()[0])
				failed.append(this_file)
...
def fmask_to_ltmask2(fmask_dir, fmask, out_dir, proj_file, mask_snow):
	"""convert fmask to icetrendr cloud mask
		...
		# outside = 255
	"""
	check_this_mask = os.path.join(out_dir, fmask[0:9] + '_' + fmask[9:13] + '_' + fmask[13:16] + '*_cloudmask.bsq')
	...
	#ltmask_base = fmask[0:9] + '_' + fmask[9:13] + '_' + fmask[13:16] + '_' + datetime.now().strftime('%Y%m%d_%H%M%S')
	ltmask_base = fmask[0:9] + '_' + fmask[9:13] + '_' + fmask[13:16]
	ltmask = os.path.join(fmask_dir, ltmask_base + '_raw.bsq')
...
def processFmask2(img_dir, out_dir, proj_file, mask_snow):
	"""Process all Fmask in img_dir to icetrendr mask"""
	failed = []
	for directory, dirnames, filenames in os.walk(img_dir):
	    for f in filenames:
		if (f.find('_cfmask.tif') < 0): continue
		...
		try:
		    print("Processing " + f)
		    fmask_to_ltmask2(directory, f, this_output, proj_file, mask_snow)
		except:
		    print(sys.exc_info()[0])
