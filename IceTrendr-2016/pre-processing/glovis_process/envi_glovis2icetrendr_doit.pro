FUNCTION envi_glovis2icetrendr_doit, inpath, outpath, overwrite=overwrite, proj=proj, skipThermal=skipThermal, pixelSize=pixelSize, from_auto=from_auto
  ...
  itnr = file_basename(inpath)
  glovis = {year:strmid(itnr, 9,4), doy :strmid(itnr,13,3), sensor :strmid(itnr,1,2), $
              path:strmid(itnr, 4,2), row :strmid(itnr, 7,2)}
 
  model = fix(strmid(itnr,2,1))
  ...
  return, {ok:1, message:'Finished '+itnr+' without errors.', warnings:warnings}
