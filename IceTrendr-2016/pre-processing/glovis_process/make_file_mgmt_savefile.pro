pro make_file_mgmt_savefile, path
  ...
  for i=0, n_targz-1 do begin
    ...
    file_mgmt[i].itimgdir = path+"images\"
    file_mgmt[i].itmadcaldir = path+"madcal\"
    ...
    file_mgmt[i].itsensor = strmid(targzbase,0,3)
    ...
    if strmid(file_mgmt[i].itsensor, 2,1) eq '7' then file_mgmt[i].vctsensor = 'L71'
    if strmid(file_mgmt[i].itsensor, 2,1) eq '5' then file_mgmt[i].vctsensor = 'L5'
    
    file_mgmt[i].itimgbase = strcompress(file_mgmt[i].itsensor+file_mgmt[i].ppprrr+"_"+file_mgmt[i].year+$
    ...
    if n_vct_img_list eq 1 then begin
      ...
      if n_vctimg ge 1 then file_mgmt[i].unpackedin = "vct" else file_mgmt[i].unpackedin = "it"
    endif else begin
  ...
  save, file_mgmt, filename = strcompress(file_mgmt[0].itimgdir+"icetrendr_file_mgmt_"+file_mgmt[0].ppprrr+".sav", /rem)
end
