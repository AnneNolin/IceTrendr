;this program will delete any trace of icetrendr files and

pro delete_it_files, path, ppprrr, date, nowarning=nowarning
  ...
  for i=0, n_elements(date)-1 do begin
    ...
    itsearchfor = strcompress("*"+year+"_"+julday+"*", /rem)
    ...
    imagefiles = file_search(path, itsearchfor, count=n_imagefiles)
        ...
        if a eq '2' then begin
          ...
          if b eq '2' then begin
            repeat begin
              ...
              imagefiles = file_search(searchdir, itsearchfor, count=n_imagefiles)
    ...
    if n_norm_info_file ne 0 then begin
      ...
      if n_norm_info_sav ne 0 and n_norm_info_csv ne 0 then begin
        ...
        summarygoods = where(strmatch(norm_info.depfile, itsearchfor) eq 1, n_summaryloc)
        if n_summaryloc ne 0 then summarygoods = where(strmatch(norm_info.depfile, itsearchfor) eq 0)
