function make_metadata_for_preprocessing, inputfile, modisimgsrc=modisimgsrc, madcal_summary=madcal_summary, cost=cost,$
  ...
  file_mgmt_file = file_search(imgdir, "*icetrendr_file_mgmt*", count=n_file_mgmt_file)
  if n_file_mgmt_file eq 1 then restore, file_mgmt_file
  image_info_file = file_search(imgdir, "*icetrendr_image_info*", count=n_image_info_file)
  ...
  case i of
    ...
    1: begin
      ...
      if file_mgmt[thisdate].unpackedin eq "lt" then begin
        ...
        it_source_filename = file_basename(archvfile[i].archvfilename)
      endif
      ...
      if keyword_set(cldmaskthresh) eq 1 then begin
        ;it_source_filename = stringswap(file_basename(inputfile), "cloudmask", "archv")
        it_source_filename = file_basename(file_search(maddir, "*cloud_shadow_reference_image.bsq"))
      ...
      metadata = {data: "cloudmask",$
        ...
        it_source_filename: it_source_filename,$
