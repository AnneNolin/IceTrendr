...
def it_basename(basename, appendix='_ledaps.bsq', with_ts=True):
	...
	if 'WO' in basename:
                if 'IT5' in basename:
                        #lndsr.IT5045029008422650_WO.hdf

                        return "IT5"+basename[9:15]+'_19'+basename[17:19]+'_'+basename[19:22]+'_'+ts+appendix
                if 'IT4' in basename:
                        return "IT4"+basename[9:15]+'_19'+basename[17:19]+'_'+basename[19:22]+'_'+ts_appendix
	if 'L4' in basename:
		baseyear = basename[20:22]
		basemonth = basename[22:24]
		baseday = basename[24:26]
		basedate=time.strptime(baseyear+" "+basemonth+" "+baseday, "%y %m %d")
		return "IT4"+basename[8:14]+'_'+basename[18:22]+'_'+str(basedate[7])+'_'+ts+appendix
	if 'LE7' in basename:
	...
	if 'IT5' in basename:
		return "IT5"+basename[9:15]+'_'+basename[15:19]+'_'+basename[19:22]+'_'+ts+appendix
	if 'IT4' in basename:
		return "IT4"+basename[9:15]+'_'+basename[15:19]+'_'+basename[19:22]+'_'+ts+appendix
	else:
		baseyear = basename[20:22]
		basemonth = basename[22:24]
		baseday = basename[24:26]
		basedate=time.strptime(baseyear+" "+basemonth+" "+baseday, "%y %m %d")
		return "IT5"+basename[8:14]+'_'+basename[18:22]+'_'+str(basedate[7])+'_'+ts+appendix
...
def extract_hdf(hdf_file, output_path, proj_file, temp_path=''):
	...
	elif 'IT5' in basefile:
		image_year = basefile[15:19]
	elif 'IT4' in basefile:
		image_year = basefile[15:19]
	else: 
...
def processIcetrendrTC(img_dir, search='_ledaps.bsq'):
	"""process all reflectance image to tc
		by defauit assuming file ends with _ledaps.bsq
	"""
	failed = []
...
def fmask_to_itmask(fmask_dir, fmask, out_dir, proj_file):
	...
	#IT5045029_1988_205_20120124_082324_cloudmask.bsq
	basename = os.path.basename(fmask_dir)
	itmask_base = basename[0:9] + '_' + basename[9:13] + '_' + basename[13:16] + '_' + datetime.now().strftime('%Y%m%d_%H%M%S')
	itmask = os.path.join(out_dir, itmask_base + '_raw.bsq')

	ds_itmask = ds_fmask.GetDriver().Create(itmask, ds_fmask.RasterXSize, ds_fmask.RasterYSize, 1, gdalconst.GDT_Byte)
	ds_itmask.SetGeoTransform(ds_fmask.GetGeoTransform())
	ds_itmask.SetProjection(ds_fmask.GetProjection())
	...
	for y in range(ds_fmask.RasterYSize):
		...
		ds_itmask.GetRasterBand(1).WriteArray(lm, 0, y)
	ds_fmask = None
	ds_itmask = None

	itmask2 = os.path.join(out_dir, itmask_base + '_cloudmask.bsq')

	#reproject to albers
	warp_cmd = 'gdalwarp -of ENVI -t_srs '+ proj_file +' -tr 30 30 {0} {1}'
	os.system(warp_cmd.format(itmask, itmask2))
	os.unlink(itmask)
	os.unlink(os.path.join(out_dir, itmask_base + '_raw.hdr'))
	...
	for directory, dirnames, filenames in os.walk(img_dir):
		for f in filenames:
			if f.endswith('Fmask'):
				...
				try:
					print("Processing " + f)
					fmask_to_itmask(directory, f, this_output, proj_file)
				except:
					print(sys.exc_info()[0])
