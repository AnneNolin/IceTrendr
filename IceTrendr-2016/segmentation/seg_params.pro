function seg_params, path, ppprrr, segparamstxt, image_info_savefile, mask_image=mask_image, subset=subset, eval=eval
  ...
  for i=0, n_runs-1 do begin
    ...
    if theruns[i].run_name ne 'na' then theruns[i].output_base = path+"outputs\"+theruns[i].index+"\IT_"+icetrendr_version()+"_"+theruns[i].index+"_"+ppprrr+"_"+theruns[i].run_name else $
      theruns[i].output_base = path+"outputs\"+theruns[i].index+"\IT_"+icetrendr_version()+"_"+theruns[i].index+"_"+ppprrr
