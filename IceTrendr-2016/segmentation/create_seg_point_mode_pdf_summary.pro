pro create_seg_point_mode_pdf_summary, path, pdf_size_params, basename, include_p_and_f=include_p_and_f
  ...
  src_file_all = file_search(path+"outputs\it_point_mode\", basename+"*src.csv", count=n_src_file)
  vert_file_all = file_search(path+"outputs\it_point_mode\", basename+"*vertinfo.csv", count=n_vert_file)
  ...
  for i=0, n_src_file-1 do begin
    ...
    lines = [lines,'out_file = paste(outdir, '+pdfout+', sep="")']  ;     "it_segments_", index, ".pdf"
